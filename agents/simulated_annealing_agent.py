...
def evaluate_position(pos, enemies, items, bricks, hazard_zones, explosions,
    ...
    for ex,ey in enemies:
        ...
        if ammo > 0: score += 400.0/(dist+1)
        else: score -= 200.0/(dist+1)
    for bx,by in bricks:
        dist = abs(px-bx)+abs(py-by)
        if dist == 1: score += 100.0
    return score
...
class SimulatedAnnealingAgent(BaseAgent):
    ...
    def _get_current_temp(self) -> float:
        """Calculate current temperature using exponential cooling."""
        return max(self.min_temp, self.temperature * (self.cooling_rate ** self.step_count))
    ...
    def choose_action(self, state: dict) -> str:
        ...
        # Safety override: always escape danger
        ...
        # Bomb placement with escape verification
        ...
        # Simulated Annealing move selection
        ...
        for action, dx, dy in moves:
            ...
            if (nx,ny) in explosions: continue
            score = evaluate_position((nx,ny), enemies, items, bricks, hazard_zones, explosions,
                                      ammo, blast_radius, width, height)
        ...
        # SA decision: accept better move always, accept worse with probability
        best_action = "WAIT"
        ...
        # Pick the best, but with SA probability also consider suboptimal
        if len(candidates) > 0:
            top_action, top_score = candidates[0]
            best_action = top_action
            best_score = top_score

            # Try a random candidate with SA probability (exploration)
            rand_action, rand_score = random.choice(candidates)
            delta_e = rand_score - current_score
            if delta_e > 0:
                best_action = rand_action  # Better always accepted
            else:
                # Accept worse move with Boltzmann probability
