...
# Hàm lượng giá Heuristic Kỳ vọng (Expectimax Evaluation)
# Đánh giá độ tốt/xấu của ô đứng dựa trên xác suất di chuyển của kẻ địch (Chance Nodes)
def heuristic_evaluate_expectimax(pos: Tuple[int, int], state_info: dict, placed_bomb: Tuple[int, int, int] = None) -> float:
    px, py = pos
    width, height = state_info["width"], state_info["height"]
    
    # 1. Tránh tuyệt đối ô đang có lửa nổ hoạt động
    if pos in state_info["explosions"]:
        return -100000.0
        
    # 2. Đánh giá tính an toàn: Phải có ít nhất 1 đường thoát hiểm khỏi vùng bom nổ
    ...
    if not check_escape_path(pos, state_info["walls"], state_info["bricks"], bombs_to_check, state_info["blast_radius"], state_info["explosions"], width, height):
        return -80000.0  # Phạt cực nặng nếu bị kẹt/không thể thoát hiểm
        
    # 3. Phạt nếu đứng trong vùng ảnh hưởng nguy hiểm của bom (timer càng nhỏ phạt càng nặng)
    ...
    # 4. Mô hình hóa Kẻ địch như các nút Cơ hội (Chance Nodes)
    # Giả định kẻ địch di chuyển ngẫu nhiên đều (uniform distribution) trên các ô hợp lệ kề cạnh họ.
    # Chúng ta tính:
    #   - Khoảng cách kỳ vọng (Expected distance) đến kẻ địch.
    #   - Xác suất va chạm kỳ vọng (Expected collision probability) nếu kẻ địch đi vào ô ta đứng.
    expected_enemy_distances = []
    collision_penalty = 0.0
    
    for ex, ey in state_info["enemies"]:
        # Các nước đi khả dĩ của kẻ địch ở bước tiếp theo (đứng yên + di chuyển kề cạnh)
        possible_enemy_moves = [(ex, ey)]
        for dx, dy in [(0, 1), (0, -1), (1, 0), (-1, 0)]:
            nx, ny = ex + dx, ey + dy
            if 0 <= nx < width and 0 <= ny < height:
                if (nx, ny) not in state_info["walls"] and (nx, ny) not in state_info["bricks"]:
                    # Kẻ địch sẽ tránh đi trực tiếp vào bom
                    if (nx, ny) not in {(b[0], b[1]) for b in bombs_to_check}:
                        possible_enemy_moves.append((nx, ny))
                        
        num_moves = len(possible_enemy_moves)
        total_distance = 0.0
        
        for mx, my in possible_enemy_moves:
            dist = abs(px - mx) + abs(py - my)
            total_distance += dist
            
            # Nếu nước đi của kẻ địch dẫn đến va chạm với ta
            if px == mx and py == my:
                # Xác suất va chạm của kẻ địch này là 1 / num_moves
                prob_collision = 1.0 / num_moves
                # Phạt nặng nhân với xác suất va chạm (Expectation of collision damage)
                collision_penalty += prob_collision * 5000.0
                
        expected_dist = total_distance / num_moves
        expected_enemy_distances.append(expected_dist)
        
    # Áp dụng phạt va chạm kỳ vọng
    score -= collision_penalty
    
    # Lấy khoảng cách kỳ vọng ngắn nhất tới bất kỳ kẻ địch nào
    min_expected_enemy_dist = min(expected_enemy_distances) if expected_enemy_distances else float('inf')
    
    # 5. Phản ứng cự ly dựa trên cự ly kỳ vọng
    if min_expected_enemy_dist <= 1.2:
        if state_info["ammo"] > 0:
            score += 2500.0  # Tấn công mạnh mẽ khi rất gần kẻ địch và có bom
        else:
            score -= 3500.0  # Chạy trốn khẩn cấp nếu ở quá gần mà không có bom
    elif min_expected_enemy_dist <= 2.2:
        if state_info["ammo"] > 0:
            score += 1200.0
        else:
            score -= 500.0
    elif min_expected_enemy_dist <= 3.2:
        if state_info["ammo"] > 0:
            score += 600.0
        else:
            score -= 60.0
    ...
    # Chạy BFS tìm khoảng cách ngắn nhất đến các mục tiêu tĩnh
...
# Lớp chính định nghĩa ExpectimaxAgent
class ExpectimaxAgent(BaseAgent):
    """An agent that uses Expectimax search with expected heuristic evaluation of opponent moves to choose actions."""
    ...
    def choose_action(self, state: dict) -> str:
        ...
        # 1. Duyệt qua các hành động khả thi ở Nút Quyết Định Bước 1 (Maximizer Node)
        for action1 in legal_actions:
            px, py = state_info["player_pos"]
            
            # Cắt tỉa (Pruning) các hành động va chạm tường/gạch ngay lập tức
            ...
            # Giả lập trạng thái game sau khi ta thực hiện hành động ở bước 1
            state_info_1 = simulate_state_forward(state_info, action1, step=1)
            # Đánh giá bằng hàm heuristic kỳ vọng (tính toán dựa trên xác suất di chuyển của kẻ địch)
            score_1 = heuristic_evaluate_expectimax(state_info_1["player_pos"], state_info_1)
            
            # Lưu lại nước đi dự phòng an toàn nhất phòng khi các bước sau đều dẫn tới ngõ cụt
            if score_1 > fallback_score:
                fallback_score = score_1
                fallback_action = action1
                
            # Nếu nước đi này chắc chắn dẫn tới cái chết ngay lập tức, bỏ qua nhánh này
            if score_1 <= -50000.0:
                continue
                
            # 2. Duyệt tiếp Bước 2 (Maximizer Node tiếp theo - Depth 2)
            ...
            for action2 in ["UP", "DOWN", "LEFT", "RIGHT", "BOMB", "WAIT"]:
                ...
                # Đánh giá kỳ vọng của trạng thái kết quả
                score = heuristic_evaluate_expectimax(state_info_2["player_pos"], state_info_2)
                
                # Điểm thưởng nếu ta đặt bom tấn công hiệu quả ở bước 1
                if action1 == "BOMB":
            ...
            # Cập nhật hành động tốt nhất có kỳ vọng cao nhất
            if max_score_2 > best_score:
                best_score = max_score_2
                best_action = action1
                
        # Nếu tất cả các nước đi đều nguy hiểm chết người, sử dụng nước đi dự phòng an toàn nhất
